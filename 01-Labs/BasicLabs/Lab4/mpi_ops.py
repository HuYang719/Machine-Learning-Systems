...
def _allreduce_async(tensor, output, name, op, prescale_factor, postscale_factor, process_set: ProcessSet):
    ...
    if tensor.dtype == torch.int8 or tensor.dtype == torch.float16:
        ...
        handle = getattr(mpi_lib, function)(
            tensor, output, name.encode() if name is not None else _NULL, process_set.process_set_id)
    else:
...
def allreduce_async_(tensor, average=None, name=None, op=None,
    ...
    """
    op = handle_average_backwards_compatibility(op, average)
    ret = _allreduce_async(tensor, tensor.new(), name, op, prescale_factor, postscale_factor, process_set) if tensor.dtype == torch.int8 or tensor.dtype == torch.float16 else _allreduce_async(tensor, tensor, name, op, prescale_factor, postscale_factor, process_set)
    return ret
